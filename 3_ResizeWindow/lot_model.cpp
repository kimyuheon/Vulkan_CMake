...
namespace lot {
    LotModel::LotModel(LotDevice &device, const std::vector<Vertex> &vertices) 
    : lotDevice(device) {
    ...
    void LotModel::createVertexBuffers(const std::vector<Vertex> &vertices) {
        ...
        lotDevice.createBuffer(bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, 
                               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                               vertexBuffer, vertexBufferMemory);

        // CPU의 vertices를 GPU로 보내기 위해 공간 확보 후 복사 -> 해제
        void *data;
        vkMapMemory(lotDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
        memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
        vkUnmapMemory(lotDevice.device(), vertexBufferMemory);
    }
